...
import com.focustech.oss2008.web.OssHandlerMethodInvoker;
import com.focustech.oss2008.web.ServletAnnotationMappingUtils;

/**
 * <li></li>
 *
 * @author yangpeng 2008-7-7 下午02:28:44
 */
public class OssAnnotationMethodHandlerAdapter extends WebContentGenerator implements HandlerAdapter {
    /**
    ...
    private List<WebBindingInitializer> webBindingInitializers;
    ...
    public OssAnnotationMethodHandlerAdapter() {
     ...
     * same client.
     * <p>
     * More specifically, the execution of each handler method will get synchronized if this flag is "true". The best
     * available session mutex will be used for the synchronization; ideally, this will be a mutex exposed by
    ...
    /**
     * Set one or more custom ArgumentResolvers to use for special method parameter types. Any such custom
     * ArgumentResolver will kick in first, having a chance to resolve an argument value before the standard argument
     * handling kicks in.
     */
    ...
    private ModelAndView invokeHandlerMethod(HttpServletRequest request, HttpServletResponse response, Object handler)
            throws Exception {
        try {
            ...
            methodInvoker.updateSessionAttributes(handler, (mav != null ? mav.getModel() : null), implicitModel,
                    webRequest);
            return mav;
        }
     ...
     * <p>
     * The default implementation logs a warning and sends an HTTP 404 error. Alternatively, a fallback view could be
     * chosen, or the NoSuchRequestHandlingMethodException could be rethrown as-is.
     *
    ...
    protected ServletRequestDataBinder createBinder(HttpServletRequest request, Object target, String objectName)
        ...
        binder.setBindingErrorProcessor(new OssBindingErrorProcessor());
        return binder;
    }
            ...
            if (targetHandlerMethods.size() == 1) {
                ...
                for (RequestMappingInfo mapping : targetHandlerMethods.keySet()) {
                    ...
                    if (bestMappingMatch == null) {
                        ...
                        if ((mappedPath != null && (bestPathMatch == null || mappedPath.equals(lookupPath) || bestPathMatch
    ...
    private class ServletHandlerMethodInvoker extends OssHandlerMethodInvoker {
        private boolean responseArgumentUsed = false;

        public ServletHandlerMethodInvoker(HandlerMethodResolver resolver) {
            super(resolver, webBindingInitializers, sessionAttributeStore, parameterNameDiscoverer,
        ...
        protected WebDataBinder createBinder(NativeWebRequest webRequest, Object target, String objectName)
                throws Exception {
            return OssAnnotationMethodHandlerAdapter.this.createBinder((HttpServletRequest) webRequest
