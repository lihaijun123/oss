...
public class SpringPropertyHandlerMapping extends PropertyHandlerMapping {
    ...
    protected void registerPublicMethods(String beanName, final ApplicationContext context) throws XmlRpcException {
        ...
        if (service != null) {
            serviceNum++;
            log.error("service " + serviceType + " has XmlRpcService annotation!");
            Map map = new HashMap();
            ...
            for (Iterator iter = map.entrySet().iterator(); iter.hasNext();) {
                ...
                log.error("add handler bean :" + beanName + ". handler array is :" + Arrays.toString(mArray));
                // System.out.println("add handler bean :" + beanName + ". handler array is :" +
                // Arrays.toString(mArray));
            }
        }
        log.error("xml rpc service num:" + serviceNum);
    }
