...
function JSDragDropTree() {
	...
	this.imageFolder = '/js/drag-drop-folder-tree/images/';
...
JSDragDropTree.prototype = {
	 ...
	 *
	 * @param Object
	 *            whichObject = Reference to HTML element(Which object to
	 *            assigne the event)
	 * @param String
	 *            eventType = Which type of event, example "mousemove" or
	 *            "mouseup"
	 * @param functionName =
	...
	setRootParentId : function(idOfRootParent) {
		this.idOfRootParent = idOfRootParent;
	},
	expandAll : function() {
	...
	dropDragableNodes : function() {
		...
		if (JSTreeObj.dragNode_destination) {
			if (JSTreeObj.insertAsSub) {
				...
				if (uls.length > 0) {
					...
					if (lis.length > 0) { // Sub elements exists - drop
						// dragable node before the first
						// one
						ul.insertBefore(JSTreeObj.dragNode_source, lis[0]);
					} else { // No sub exists - use the appendChild method -
						// This line should not be executed unless
						// there's something wrong in the HTML, i.e
						// empty <ul>
		...
		//检测移动的节点是否可以移动（如預告信息2、新功能3节点不可以移动）
		var sourceId = JSTreeObj.dragNode_source.id;
		if(sourceId === "2"||sourceId === "3"){
			vAlertInfo("不能拖动此节点！", "警告");
			//恢复拖动之前
			initLiAndDrag(liId);
		}else{

		//alert(JSTreeObj.dragNode_destination.id);
		var destId = JSTreeObj.dragNode_destination.id;
		//检测移动的目标节点是否已经有了关联信息
		//如果是叶子节点，则检测是否有关联的信息，有则给出提示信息
		var leafFlag = checkIsLeaf(destId);

			if(leafFlag){
				$.ajax( {
					url : "helpColumn.do?method=checkIsRelated",
					type : "GET",
					async: false,
					dataType: "json",
					data : 'key='+destId,
					success : function(result) {
						if(result.isRelated =="true") {
							vConfirm("此栏目已经有信息关联，如果添加子栏目则关联的信息类型会自动匹配到保存后的所有的终极栏目的第一个子栏目中？","",function(r){
								if(r){
							//if(confirm("此栏目已经有信息关联，如果添加子栏目则关联的信息类型会自动匹配到保存后的所有的终极栏目的第一个子栏目中")){
								changedTypeArray.push(destId);
								//alert(changedTypeArray);
								//可以拖动
								//重新刷新tree数组
								tree.length=0;
								tree = treeObj.getNodeOrders().split(",");
								initLiAndDrag(liId);
								}else{
									//恢复拖动之前
									initLiAndDrag(liId);
						...
						}else if(result.isRelated =="false"){
							//可以拖动
							//重新刷新tree数组
							tree.length=0;
							tree = treeObj.getNodeOrders().split(",");
			...
			while (li) {
				if (li.id) {
					...
					var numericParentID = li.parentNode.parentNode.id.replace(
							/[^0-9]/gi, '');
					//if (numericID != '0') {
					//if (numericParentID != this.idOfTree) {
						...
						if (li.parentNode.id != this.idOfTree)
							...
							saveString = saveString + this.idOfRootParent;
					//}
						//var a = li.getElementsByTagName('a');
//						saveString = saveString + "|" + a[0].text;
						//jquery处理
						//为空检测
						if($($("#" + numericID).children("a").get(0)).text() === "")
							continue;
						saveString = saveString + "|" + $($("#" + numericID).children("a").get(0)).text();
						//添加isDelete标志
						saveString = saveString + '|';
						saveString = saveString + $(li).attr("isdelete");

						//添加leafFlag标记 //1 代表是叶子节点  0代表不是叶子节点(与数据库保存有关)
						saveString = saveString + '|';
						if(this.hasSubNodes(li)){
							saveString = saveString +'0';
						}else{
							saveString = saveString +'1';
						}
						//排序
						saveString = saveString+ '|' + num;
	...
	deleteItem : function(obj1, obj2) {
		...
		if (confirm(message)) {
			this.__deleteItem_step2(obj2.parentNode); // Sending <LI> tag to
			// the
			// __deleteItem_step2
			// method
		}
	...
	__deleteItem_step2 : function(obj) {
		...
		JSTreeObj.ajaxObjects[ajaxIndex].requestFile = JSTreeObj.filePathDeleteItem; // Specifying
		// which
		// file
		// to
		// get
	...
	__saveTextBoxChanges : function(e, inputObj) {
		...
		if (inputObj.value.length > 0) {
			...
			JSTreeObj.ajaxObjects[ajaxIndex].requestFile = JSTreeObj.filePathRenameItem; // Specifying
			// which
			// file
			// to
			// get
	...
	initTree : function() {
		...
		if (this.deleteAllowed || this.renameAllowed) {
			try {
				...
				if (this.deleteAllowed)
					menuModel.addItem(1, 'Delete', '', '', false,
							'JSTreeObj.deleteItem');
				if (this.renameAllowed)
					menuModel.addItem(2, 'Rename', '', '', false,
							'JSTreeObj.renameItem');
				menuModel.init();

				var menuModelRenameOnly = new DHTMLGoodies_menuModel();
				if (this.renameAllowed)
					menuModelRenameOnly.addItem(3, 'Rename', '', '', false,
							'JSTreeObj.renameItem');
				menuModelRenameOnly.init();

				var menuModelDeleteOnly = new DHTMLGoodies_menuModel();
				if (this.deleteAllowed)
					menuModelDeleteOnly.addItem(4, 'Delete', '', '', false,
							'JSTreeObj.deleteItem');
		...
		var menuItems = dhtmlgoodies_tree.getElementsByTagName('LI'); // Get
		// an
		// array
		// of
		// all
			...
			if (this.contextMenu) {
				...
				if (noRename == 'true' && noDelete == 'true') {
					...
					if (noDelete == 'true')
						...
						this.contextMenu
								.attachToElement(aTag, false, menuModel);
