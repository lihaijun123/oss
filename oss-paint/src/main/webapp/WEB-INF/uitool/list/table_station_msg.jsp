...
<%
				...
				String dispValue = ResponseUtils.filter(UIToolUtils.getSubString(StringTools.valueOf(value),cellCharLength));
				boolean isSenderNameColumn = "SENDER_NAME".equals(title.getId());
				boolean isReceiverNameColumn = "RECEIVER_NAME".equals(title.getId());
				if(isSenderNameColumn || isReceiverNameColumn)
				{
					String senderMemberSn = String.valueOf(line.get("SENDER_MEMBER_SN"));
					String receiverMemberSn = String.valueOf(line.get("RECEIVER_MEMBER_SN"));
					if((isSenderNameColumn && "-1".equals(senderMemberSn)) || (isReceiverNameColumn && "-1".equals(receiverMemberSn) )){
%>
						<td class="<%=tdClass%>" align="<%=align%>"><%=dispValue %></td>
<%
					} else {
%>
						<td class="<%=tdClass%>" align="<%=align%>"><a href="<%=UIToolUtils.formatUrl(UIToolUtils.formatUrl(title.getUrl(),data1.getLineData(),encode),request,form1.getReqData(),encode)%>"><%=dispValue%></a></td>
<%
					}
				} else {
%>
