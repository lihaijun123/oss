...
<%-- 显示标题--%>
	...
	<tr align="center" bgcolor="#FFFFFF" class="<%=style%>" id="data_tr" <%=UIToolUtils.formatUrl(attrs.toString(),line)%>>
		...
		%>
				<td style="width:30px;">
					<input type="<%=m?"checkbox":"radio"%>" index="<%=index-1%>" value="<%=data1.getString("KEY")%>" text="<%=data1.getString("VALUE")%>" name="sel_id" <%=checked%>>
				</td>
		<%}%>
				<td>
					<%=line.get("NUM_ROW_ZZZ")%>
				</td>
		<%
		...
		%>
				<td class="<%=tdClass%>" title="<%=ResponseUtils.filter(String.valueOf(value))%>" align="<%=align%>" >
					<%=aTmp%>
				</td>
		<%
				} else {
		...
		%>
				<td class="<%=tdClass%>" align="<%=align%>">
					<a href="<%=UIToolUtils.formatUrl(UIToolUtils.formatUrl(title.getUrl(),data1.getLineData(),encode),request,form1.getReqData(),encode)%>">
						<%=UIToolUtils.getSubString(StringTools.valueOf(value),cellCharLength)%>
					</a>
				</td>
		<%
					} // enf if
	...
	<tr align="center" bgcolor="#FFFFFF" class="cl_tfoot" id="data_tfoot">
		...
		%>
				<td class="<%=tdClass%>" title="<%=value%>" align="<%=align%>" nowrap>
					<%=UIToolUtils.getSubString(StringTools.valueOf(value),cellCharLength)%>
				</td>
		<%
			} // for ---
