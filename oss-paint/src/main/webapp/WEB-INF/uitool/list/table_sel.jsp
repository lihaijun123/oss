			...
			if("_oper_".equalsIgnoreCase(((HtmlTDBean)cells.get(iCell)).getId())) {
				...
				if(vTexts.length >1) {
					...
					for( ; iv < vTexts.length ;iv++) {
						if(RoleResourceUtils.check(StringTools.parseLong(vTexts[iv],-1),pageContext)) {
							break;
						}
					}
					if(iv >= vTexts.length) {
...
Pattern P_MAIL = Pattern.compile("^[-\\w]+@[-\\w]+[-\\.\\w+]+(.?[-\\w]+@[-\\w]+[-\\.\\w+]+)*$", Pattern.CASE_INSENSITIVE);
Pattern P_URL = Pattern.compile("^(http://)?(www\\.)?[-\\w]+(\\.[-/\\w]+){2,}(.?(http://)?(www\\.)?[-\\w]+(\\.[-/\\w]+){2,})*", Pattern.CASE_INSENSITIVE);
while(data1.next()) {
	if(index++%2 == 0) {
		style = "odd";
	} else {
...
%>
				<td style="width:30px;"><input type="<%=multiple?"checkbox":"radio"%>" index="<%=index-1%>" value="<%=data1.getString("KEY")%>" text="<%=data1.getString("VALUE")%>" name="sel_id" <%=checked%>></td>
				<td><%=line.get("NUM_ROW_ZZZ")%></td>
<%
}
	...
	for(int j = 0 ; j < titleData.size();j++) {
		...
		String align="";
		//
		String uriTmp = StringTools.valueOf(value);
		//boolean encode0 = (uriTmp.startsWith("javascript:") || uriTmp.startsWith("http://")) || uriTmp.matches("^[-\\w]+\\@[-\\w]+[-.\\w+]+");
		//if(encode0) cellCharLength = uriTmp.length()+2;
		//
		...
		if("".equals(title.getUrl())) {
			//
			boolean isUrl = P_URL.matcher(uriTmp).matches();
			boolean isMail = false;//P_MAIL.matcher(uriTmp).matches();
			if(isUrl || isMail) cellCharLength = uriTmp.length()+2;
			String aTmp = "";
			if(isUrl) {
				String urlResult="";
				if(uriTmp!=null){
					for(String urlTemp:uriTmp.split("\t")){
						aTmp = "";
						if(!urlTemp.startsWith("http://") && !urlTemp.startsWith("https://")) {
							aTmp = "http://";
						}
						aTmp += urlTemp;
						urlResult += "<a href='" + aTmp + "' target='_blank'>"+ urlTemp + "</a><br>";
					}
					aTmp = urlResult;
				}
			}
			if(isMail) {
				...
				if(aTmp != null && aTmp.length() > 0) {
					aTmp = "<a href='" + aTmp + "'>"+ uriTmp + "</a>";
				}
			}
			//
			if(aTmp == null || aTmp.length() == 0) {
				aTmp = UIToolUtils.getSubString(StringTools.valueOf(value),cellCharLength);
			}
			//
%>
				<td class="<%=tdClass%>" title="<%=ResponseUtils.filter(String.valueOf(value))%>"><%=aTmp%></td>
<%
		} else {
				...
				if(disTexts.length > 1) {
					...
					if(!roleCache.containsKey(id)) {
						...
						if(vTexts.length >1) {
							...
							roleCache.put(id,Boolean.valueOf(RoleResourceUtils.check(actId,pageContext)));
						} else {
...
%>
				<td class="<%=tdClass%>" title="<%=StringTools.valueOf(value) %>"><a href="<%=UIToolUtils.formatUrl(UIToolUtils.formatUrl(title.getUrl(),data1.getLineData(),encode),request,form1.getReqData(),encode)%>"><%=UIToolUtils.getSubString(StringTools.valueOf(value),cellCharLength)%></a></td>
<%
			} // enf if
		} // end if
	} // end for
