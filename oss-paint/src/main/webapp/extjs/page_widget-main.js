 ...
 */
Ext.namespace('S');
S.opener = window.parent.document;
S.parentifm = function(iframe_id) {
	return window.parent.Ext.getCmp(iframe_id).getFrameDocument();
}
S.frame = function(iframe_id) {
	return Ext.getCmp(iframe_id).getFrameDocument();
}
/*
 * 對應tab里面訪問其他tab
 */
...
Ext.onReady(function() {
	...
	Ext.each(objArray_tabs, function(obj_tab) {
		...
		if (!obj_tab.attributes.getNamedItem('width')
				&& !obj_tab.attributes.getNamedItem('height')) {
			tabpanel_config.region = 'center';
			tabpanel_config.autoWidth = true;
		}
		...
		if (obj_tab.attributes.getNamedItem('height')
				|| obj_tab.attributes.getNamedItem('width')) {
...
/*
 * @class Ext.ux.ManagedIFrame Version: 1.1 Author: Doug Hendricks.
 ...
 * ***********************************************************************************
 * 
 * License: ux.ManagedIFrame and ux.ManagedIFramePanel are licensed under the
 * terms of the Open Source LGPL 3.0 license. Commercial use is permitted to the
 * extent that the code/component(s) do NOT become part of another Open Source
 * or Commercially licensed development library or toolkit without explicit
 * permission.
 * 
 ...
 * document content when rendered. @cfg {Object} loadMask An
 * {@link Ext.LoadMask} config or true to mask the iframe while using the update
 * or setSrc methods (defaults to false). @cfg {Object} src The src attribute to
 * be assigned to the Iframe after initialization (overrides the autoCreate
 * config src attribute) @constructor @param {Mixed} el, Config object The
 * iframe element or it's id to harness or a valid config object.
 * 
 * Release: 1.1 (4/13/2008) Adds Ext.Element, CSS Selectors (query,select) fly,
 * and CSS interface support (same-domain only) Adds blur,focus,unload events
 * (same-domain only)
 * 
 */
(function() {
	...
	Ext.ux.ManagedIFrame = function() {
		...
		if (el && el.dom && el.dom.tagName == 'IFRAME') {
			...
			el = config.autoCreate ? Ext.get(Ext.DomHelper.append(
					config.autoCreate.parent || document.body, Ext.apply({
						tag : 'iframe',
	...
	Ext.extend(Ext.ux.ManagedIFrame, Ext.util.Observable, {
		...
		destroy : function() {
			...
			if (this.dom) {
				...
				if (document.addEventListener) { // Gecko/Opera
					Ext.EventManager.un(window, "DOMFrameContentLoaded",
							this.dom[this._eventName]);
				}
		...
		showMask : function(msg, msgCls, forced) {
			...
			if ((lmask = this.loadMask) && (!lmask.disabled || forced)) {
				...
				lmask._vis = true;
				lmask.masker.mask.defer(lmask.delay || 5, lmask.masker, [
						msg || lmask.msg, msgCls || lmask.msgCls]);
			}
		}
		// Private
		,
		...
		loadHandler : function(e) {
			...
			switch (rstatus) {
				...
				case 'DOMFrameContentLoaded' : // Gecko, Opera
					if (this._domFired || (e && e.target !== this.dom)) {
						return;
					} // not this frame.
				case 'domready' : // MIF
					if (this._domFired)
					...
					if (this._domFired = this._hooked = this._renderHook()) {
						this._frameAction = (this.fireEvent("domready", this) === false
								? false
								: this._frameAction); // Only raise if sandBox
						// injection succeeded
						// (same domain)
					}
				case 'domfail' : // MIF
				...
				case 'complete' : // IE
					...
					if (this._frameAction || this.eventsFollowFrameLinks) {
						// not going to wait for the event chain, as its not
						// cancellable anyhow.
						this.fireEvent
								.defer(50, this, ["documentloaded", this]);
					}
		...
		checkDOM : function(win) {
			...
			var n = 0, win = win || this.getWindow(), manager = this, domReady = false, max = 100;
	...
	/*
	 * @class Ext.ux.ManagedIFramePanel Version: 1.1 (4/13/2008)
	 * 
	 * 
	...
	Ext.ux.ManagedIframePanel = Ext.extend(Ext.Panel, {
		...
		autoScroll : false,
		closable : true, /*
							 * set True by default in the event a site times-out
							 * while loadMasked
							 */
		...
		initComponent : function() {
			...
			this.bodyCfg || (this.bodyCfg = {
				...
				children : [{
					...
					,
					children : [Ext.apply(Ext.apply({
						tag : 'iframe',
						frameborder : 0,
						cls : 'x-managed-iframe',
						style : this.frameStyle || null
					}, this.frameConfig), unsup, Ext.isIE && Ext.isSecure ? {
		...
		onRender : function(ct, position) {
			Ext.ux.ManagedIframePanel.superclass.onRender.call(this, ct,
					position);
						...
						Ext.each(['north', 'south', 'east', 'west'], function(
							...
							if ((reg = layout[region]) && reg.splitEl) {
								...
								if (!reg.split._splitTrapped) {
									reg.split.on('beforeresize', MIM.showShims,
											MIM);
		...
		afterRender : function(container) {
			...
			if (this.iframe) {
				if (this.defaultSrc) {
					...
					this.iframe.update(typeof html == 'object' ? Ext.DomHelper
							.markup(html) : html);
				}
			}
		},
	...
	Ext.onReady(function() {
		...
		CSS.getRule('.x-managed-iframe')
				|| (rules
						.push('.x-managed-iframe {height:100%;width:100%;overflow:auto;}'));
		CSS.getRule('.x-managed-iframe-mask')
				|| (rules
						.push('.x-managed-iframe-mask{width:100%;height:100%;position:relative;}'));
		if (!CSS.getRule('.x-frame-shim')) {
			rules
					.push('.x-frame-shim {z-index:9000;position:absolute;top:0px;left:0px;background:transparent!important;overflow:hidden;display:none;}');
			rules
		...
		CSS.getRule('.x-hide-nosize')
				|| (rules
						.push('.x-hide-nosize,.x-hide-nosize object,.x-hide-nosize iframe{height:0px!important;width:0px!important;border:none;}'));
		if (!!rules.length) {
