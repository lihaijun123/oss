...
Ext.ux.TreeCheckNodeUI = function() {
    ...
    //單選: 'single'
    //級聯多選: 
	//'cascade'(同時選父和子，只要父節點和子節點存在就選中)
	//'extCascade'(同時選父和子,如果父節點下的子節點都選中了，則該父節點才選中，否則不選中；如果子節點存在，則所有子節點都選中)
	//'childCascade'(選子，只要子節點存在就選中)
	//'parentCascade'(選父，只要父節點存在就選中);
	//'parentExtCascade'(選父，如果父節點下的子節點都選中了，則該父節點才選中，否則不選中)
...
Ext.extend(Ext.ux.TreeCheckNodeUI, Ext.tree.TreeNodeUI, {
    ...
    check: function(checked) {
        ...
        if (this.checkModel == 'single') {
            ...
            if (this.checkModel == 'cascade' || this.checkModel == 'extCascade' || this.checkModel == 'childCascade') {
            ...
            if (this.checkModel == 'extCascade' || this.checkModel == 'parentExtCascade') {
                var parentNode = n.parentNode;
                if (parentNode !== null) {
                    this.parentExtCheck(parentNode, checked);
                }
            }
        }
    },
	...
	// private
    parentExtCheck: function(node, checked) {
        var checkbox = node.getUI().checkbox;
        if (typeof checkbox == 'undefined') 
            return;
		if (this.allChildChecked(node)) {
